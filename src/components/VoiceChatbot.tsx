...
const VoiceChatbot: React.FC<VoiceChatbotProps> = ({ isOpen, onClose, onOpenCamera }) => {
  ...
  useEffect(() => {
    ...
    if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
      ...
      addMessage('দুঃখিত, আপনার ব্রাউজার ভয়েস রেকগনিশন সাপোর্ট করে না। আপনি টেক্সট চ্যাট ব্যবহার করতে পারেন।', 'bot');
    }
  ...
  const addMessage = (text: string, sender: 'user' | 'bot', type: 'text' | 'voice' = 'text') => {
  ...
  const handleVoiceCommand = async (transcript: string) => {
    ...
    if (userMessage.includes('pregnancy support') || userMessage.includes('প্রেগনেন্সি সাপোর্ট')) {
    ...
    if (userMessage.includes('cross the road') || userMessage.includes('রাস্তা পার') || userMessage.includes('রোড পার')) {
      const response = 'রাস্তা পার হওয়ার জন্য ক্যামেরা খুলছি। আমি আপনাকে বস্তু সনাক্তকরণ এবং নেভিগেশন গাইডেন্স দেব।';
      addMessage(response, 'bot');
      speak(response);
      setTimeout(() => {
        onOpenCamera();
    ...
    setIsLoading(true);
    try {
      const aiResponse = await generateHealthResponse(userMessage, 'general-health');
      addMessage(aiResponse, 'bot');
      speak(aiResponse);
    } catch (error) {
      const errorResponse = 'দুঃখিত, কিছু সমস্যা হয়েছে। অনুগ্রহ করে আবার চেষ্টা করুন।';
            ...
            <div className="p-6 bg-gradient-to-r from-gray-50 to-purple-50 flex-shrink-0">
              <div className="flex items-center justify-center space-x-4">
                ...
                <motion.button
                  ...
                  style={{
                    boxShadow: isListening && speechSupported
